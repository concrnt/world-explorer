...
export const getConcrntDomain = async (fqdn: string): Promise<Domain> => {
  try {
    const response = await fetch(`https://${fqdn}/api/v1/domain`)
    if (!response.ok) {
      console.log("error fetch domain", fqdn)
      return {
        fqdn,
        ccid: "",
        csid: "",
    ...
    return response.json().then(data => data.content).catch(e => {
      console.log("error json domain", fqdn)
      return {
        fqdn,
        ccid: "",
        csid: "",
